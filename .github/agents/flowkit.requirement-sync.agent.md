---
description: 將 Feature 開發過程中的調整修正，同步回寫至 PRD 與 User Stories 需求文件
handoffs:
  - label: 執行 Unify Flow
...
| `/flowkit.pre-unify-check` | code-check 後 | 檢查 Spec 品質與實作對齊 |
| `/flowkit.requirement-sync` | trace 後 | 將 Feature 變更回寫至需求文件（**本指令**） |
| `/flowkit.unify-flow` | requirement-sync 後 | 合併 Feature 至 System |
...
**需求保留原則**：requirement-sync 的目的是「將本意修正寫回」，而非同步所有差異。以下兩類差異 MUST 保留原始需求，不得回寫：
- **範圍裁切**：Feature 移除的項目若屬於「暫不實作」而非「永遠不做」，原始需求應保留作為未來 Milestone 候選
- **限縮合併**：Feature 將限制條件收緊或合併為更嚴格版本時，需求文件應保持原始合理門檻（實作可以更嚴格，但需求不應被收窄）
...
- 刪除 PRD / User Stories 中屬於「範圍裁切」的內容（移除可選項、初版範圍裁剪、未來待開發項目）
- 將 Feature 的更嚴格限制條件回寫覆蓋需求文件中的原始合理門檻
...
4. **掃描輔助信號源**（用於增強意圖判斷）：
   - `plan.md`：Phase 描述中的變更決策說明
   - `FEATURE_DIR/.refine/*/refine-spec-delta.md`：若存在，提取 refine-loop 產生的變更記錄   
   - `FEATURE_DIR/spec-delta-log.md`：若存在，提取各階段記錄的規格差異日誌（Stage/Type/Impact）   
   - Analyze 報告中的 Remediation 記錄（若有）
...
| 分類 | 判斷依據 | 處理方式 |
|------|----------|----------|
| **A. 刻意修正** | Feature Spec 有明確變更標記、Plan 有說明、Refine Delta 有記錄、合理的設計調整 | 自動回寫 |
| **B. 澄清補充** | 內容更詳細但語意相同 | 建議回寫 |
| **C. 範圍裁切** | Feature 移除標記為「可選」的 AC/US、初版範圍裁剪、Feature 未實作但需求尚具未來價值的項目 | 🔒 保留不動 |
| **D. 限縮合併** | Feature 收緊限制條件（數值更嚴格）、合併多個 AC 為更嚴格的單一 AC | 🔒 保留原始 |
| **E. 非意圖不一致** | 無變更標記、無輔助信號、疑似遺漏或錯誤 | 詢問使用者 |
| **F. 格式差異** | 僅格式不同，語意相同 | 忽略 |

> 📌 **C/D 類保留原則**：requirement-sync 的目的是「將本意修正寫回」。範圍裁切（暫不做 ≠ 永遠不做）與限縮合併（實作更嚴格 ≠ 需求應收窄）不屬於本意修正，MUST 保留原始需求內容。

> 📌 **多信號源判斷**：即使 spec.md 中無 `[MODIFIED]` 標記，若 plan.md、refine-spec-delta.md 或 spec-delta-log.md 中有對應的變更說明，仍可判定為「刻意修正」。
...
FOR each difference:
  ...
  IF 差異為「Feature 移除 AC/US」:
    IF 原始內容標記為「可選」或「選配」:
      → 分類為「C. 範圍裁切」（保留不動）
    ELSE IF 移除原因為初版範圍裁剪、暫不實作、未來 Milestone:
      → 分類為「C. 範圍裁切」（保留不動）
    ELSE IF 移除是明確的設計決策（永久移除）:
      → 分類為「A. 刻意修正」（回寫刪除）

  ELSE IF 差異為「限制條件更嚴格」:
    IF Feature 合併多個 AC 為更嚴格的單一 AC:
      → 分類為「D. 限縮合併」（保留原始）
    ELSE IF Feature 收緊數值門檻（如 ≤400MB → <350MB）:
      → 分類為「D. 限縮合併」（保留原始）
    ELSE IF 收緊是需求層面的修正（如錯誤修正、安全需求）:
      → 分類為「A. 刻意修正」（回寫更新）

  ── Step 2：標準判斷流程 ──

  ELSE IF Feature Spec 有 [MODIFIED] 標記:
    → 分類為「A. 刻意修正」
  ELSE IF Plan 有相關變更說明:
    → 分類為「A. 刻意修正」
  ELSE IF refine-spec-delta.md 中有對應的變更記錄:
    → 分類為「A. 刻意修正」
  ELSE IF spec-delta-log.md 有對應記錄（比對 Impact 欄位）:
    → 分類為「A. 刻意修正」
...
> 📌 **Step 1 優先原則**：「範圍裁切」與「限縮合併」的判斷 MUST 在標準判斷之前執行，避免被 `[MODIFIED]` 標記或 Plan 說明誤判為「刻意修正」而回寫。
...
### A. 刻意修正（自動回寫）— N 項
| # | 項目 | 原始內容摘要 | 修正後內容摘要 | 信號源 |
|---|------|--------------|----------------|--------|
| 1 | US-A-1 AC2 | Given... | Given...（更新） | plan.md |

### B. 澄清補充（建議回寫）— N 項
| # | 項目 | 原始內容摘要 | 補充內容摘要 |
|---|------|--------------|--------------|
| 1 | US-B-1 標題 | 功能描述 | 更詳細的描述 |

### C. 範圍裁切（🔒 保留不動）— N 項
| # | 項目 | 原始內容摘要 | Feature 處理 | 保留原因 |
|---|------|--------------|--------------|----------|
| 1 | US-X-N ACn | 原始 AC 內容 | Feature 移除 | 可選功能，未來 Milestone 候選 |

### D. 限縮合併（🔒 保留原始）— N 項
| # | 項目 | 原始內容摘要 | Feature 調整 | 保留原因 |
|---|------|--------------|--------------|----------|
| 1 | US-X-N ACn | ≤ 400MB | < 350MB（更嚴格） | 需求保持合理門檻，實作可更嚴格 |

### E. 非意圖不一致（需確認）— N 項
| # | 項目 | 原始內容摘要 | Feature 內容摘要 | 建議 |
|---|------|--------------|------------------|------|
| 1 | US-C-1 AC1 | Given X | Given Y | 確認哪個正確 |
...
**C/D 類自動保留（無需確認）**：
- 「C. 範圍裁切」→ 自動保留原始需求，不回寫，記錄保留原因
- 「D. 限縮合併」→ 自動保留原始需求，不回寫，記錄保留原因
...
**若使用 `--auto` 參數**：
- 「A. 刻意修正」和「B. 澄清補充」自動回寫
- 「C. 範圍裁切」和「D. 限縮合併」自動保留不動
- 「E. 非意圖不一致」跳過並記錄
...
4. **刪除項目處理**（僅限 A 類「刻意修正」中的永久移除）：
   - 若 Feature 標記刪除某 US **且** 確認為永久移除（非範圍裁切），在需求文件中標記為「已移除」或刪除
   - 更新 README.md 索引
   - ⚠️ 若為 C 類「範圍裁切」，MUST NOT 刪除，保留原始內容
...
| A. 刻意修正（已回寫） | N | 自動回寫完成 |
| B. 澄清補充（已回寫） | N | 經確認回寫 |
| C. 範圍裁切（🔒 保留） | N | 保留原始需求，未來 Milestone 候選 |
| D. 限縮合併（🔒 保留） | N | 保留原始合理門檻 |
| E. 非意圖不一致（已處理） | N | 經使用者決策 |
| E. 非意圖不一致（跳過） | N | 需人工處理 |
| F. 格式差異（忽略） | N | 無需處理 |
...
- [ ] 所有「A. 刻意修正」項目已回寫
- [ ] 所有「B. 澄清補充」項目已處理（回寫或跳過）
- [ ] 所有「C. 範圍裁切」項目已確認保留（未被刪除）
- [ ] 所有「D. 限縮合併」項目已確認保留原始值
- [ ] 「E. 非意圖不一致」項目已決策或記錄
- [ ] 修改的檔案語法正確
