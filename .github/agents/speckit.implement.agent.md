...
### 10.5 Spec Delta Log 規格差異日誌（MUST）

**在實作過程中**，若發現實隞情況與 spec/plan/tasks 不一致，MUST 記錄到 `FEATURE_DIR/spec-delta-log.md`（不中斷實作流程）：
...
> 📌 此 Spec Delta Log 供 `refine-loop`（修正）、`requirement-sync`（意圖判斷）、`pre-unify-check`（品質檢查）使用。
