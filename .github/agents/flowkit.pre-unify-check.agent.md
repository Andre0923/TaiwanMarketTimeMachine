...
**N/A 嚴格適用原則**：N/A 僅適用於「結構性不適用」（如全新功能無需對齊檢查），不得因「資訊不足」或「難以判斷」而標記 N/A。遇到資訊不足時，MUST 標記為 ⚠️ WARNING 並說明缺少的資訊，預設建議修正。
...
**全新功能時**：T1-T3 標記 N/A（全新功能不存在「範圍對齊」問題，屬結構性不適用）。其他檢查項目即使資訊不完整也 MUST NOT 標記 N/A，應以 ⚠️ WARNING 呈現。
...
> ⚠️ N/A 僅適用於「結構性不適用」，不得因資訊不足而標記。資訊不足時應歸入「警告」。
